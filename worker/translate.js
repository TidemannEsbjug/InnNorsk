...
// strings.json (alt som skal oversettes) og b-<idx>.json per ferdig batch, så ingen batch betales to ganger.
// En fil eies av instansen som tok den (files.workflow_id); en instans som har mistet filen, lar den være.
...
async function prepare(env, owner, fileId) {
  ...
  return inStep(env, fileId, owner, async () => {
    ...
    const batches = strings
      .flatMap((list, c) => grok.planBatches(list).map((b, k) => ({ c, k, chars: b.chars })))
      .map((b, idx) => ({ idx, ...b }));
...
async function translateChunk(env, owner, fileId, { targetLanguage, batches }, n) {
  ...
  return inStep(env, fileId, owner, async () => {
    ...
    const itemsOf = (b) => {
      ...
      return plans.get(b.c)[b.k].items;
    };
    const calls = [];
